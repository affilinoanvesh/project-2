...
const Settings: React.FC = () => {
  ...
  const [syncType, setSyncType] = useState<'all' | 'products' | 'year' | 'custom'>('all');
  ...
  const syncState = useOperationState();
  ...
  const handleSyncData = async () => {
    ...
    try {
      ...
      // Create a simple progress update function that only passes a number
      const updateProgress = (progress: number) => {
        syncState.updateProgress(progress);
      };
      
      // Sync data based on selected type
      if (syncType === 'products') {
        // Sync only products
        await syncProductsOnly(updateProgress);
      } else if (syncType === 'year') {
        // Sync orders for the selected year
        const startDate = new Date(syncYear, 0, 1);
        const endDate = new Date(syncYear, 11, 31, 23, 59, 59);
        
        console.log(`Syncing orders for year ${syncYear} (${startDate.toISOString()} to ${endDate.toISOString()})`);
        
        updateProgress(10);
        await syncOrdersByYear(startDate.toISOString(), endDate.toISOString(), updateProgress);
      } else if (syncType === 'custom') {
        // Sync orders for the custom date range
        const startDate = new Date(syncStartDate);
        const endDate = new Date(syncEndDate);
        endDate.setHours(23, 59, 59);
        
        console.log(`Syncing orders for custom range (${startDate.toISOString()} to ${endDate.toISOString()})`);
        
        updateProgress(10);
        await syncOrdersByDateRange(startDate.toISOString(), endDate.toISOString(), updateProgress);
      } else {
        // Sync all data (default)
        console.log('Syncing all data');
        updateProgress(10);
        await syncAllData(updateProgress);
      }
      ...
      setErrorMessage('Failed to sync data: ' + (error instanceof Error ? error.message : String(error)));
      syncState.reset();
    }
  };
